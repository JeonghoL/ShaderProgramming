...
void Falling()
{
	...
	if (newTime > 0)
	{
		float lifeTime = (a_randomValue2 * 0.1);		// �̰� ��ȣ ä��� ���蹮��
		float t = (mod(newTime, lifeTime));				// �̰� ��ȣ ä��� ���蹮��
		float scale = pseudoRandom(a_randomValue1) * (lifeTime - t) / lifeTime;		// (lifeTime - t) / lifeTime �� (1 - mod(lifeTime, 1.0)) �� ���������� ?
...
void Thrust()
{
	...
	if (newTime > 0)
	{
		...
		float amp =  (a_randomValue) * ampScale;
		float period = a_randomValue2;

		float scale = t * 2.0;

		vec4 newPosition;
		newPosition.x = a_Position.x * scale + 2 * t - 1;			// �̷��� (������� ���� -> �� ���ʿ��� ����)�� ���蹮�� ����
		newPosition.y = a_Position.y * scale + amp * sin(t * 2 * PI * period);
		newPosition.z = a_Position.z;
		newPosition.w = 1.0;
		gl_Position = newPosition;
		v_Grey = 1 - t;
...
void Fire()
{
	...
	const vec2 FireGravity = vec2(0, -0.4); // 불은 올라가지만 점점 느려짐

	float newTime = u_Time - a_randomValue1; // 파티클마다 다른 시작 시간

	if (newTime > 0.0)
	{
		float t = mod(newTime, LIFETIME);
		v_Grey = t / LIFETIME; // 0=새것(뜨거움), 1=오래됨(식어감)

		// 위쪽 좁은 방향으로 퍼지는 불꽃 (±30도)
		float angle = a_randomValue * PI / 6.0;
		float speed = 1.0 + pseudoRandom(a_randomValue) * 0.8;
		float vx = sin(angle) * speed * 0.4;
		float vy = cos(angle) * speed;

		// 시간에 따른 수평 흔들림 (불꽃이 바람에 흔들리는 효과)
		float turbulence = sin(t * PI * 5.0 + a_randomValue * PI * 2.0) * 0.04 * t;

		vec4 newPosition;
		newPosition.x = a_Position.x + vx * t + turbulence;
		newPosition.y = a_Position.y - 0.8 + vy * t + 0.5 * FireGravity.y * (t * t);
		newPosition.z = 0;
		newPosition.w = 1.0;
		gl_Position = newPosition;
	}
	...
	{
		v_Grey = 0.0;
		gl_Position = vec4(10000, 100, 0, 1.0); // 화면 밖으로
	}
}
